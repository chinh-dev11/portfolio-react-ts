...
const createSkillsNodes = (
  skillsData: IconProps[],
  cbCreateIcon: Function
  // cbCreatePillList: Function
): ReactNode => {
...
export default () => {
  ...
  const skillsData: IconProps[] = Object.values(list.fields).map(
    (skill: any) => skill.fields
  )

  return (
    <section className="py-20 app-align-element" id="skills">
      <h3 className="text-3xl font-bold capitalize border-b border-slate-300 pb-5 mb-16">
        {blockData.title}
      </h3>
      {createSkillsNodes(skillsData, createIcon)}
    </section>
  )
}
